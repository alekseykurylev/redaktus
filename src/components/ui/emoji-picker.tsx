...
function EmojiPicker({
  ...
  return (
    <EmojiPickerPrimitive.Root
      className={cn(
        "isolate flex h-full w-fit flex-col overflow-hidden rounded-md bg-background text-popover-foreground",
        className,
      )}
...
function EmojiPickerEmoji({ emoji, className, ...props }: EmojiPickerListEmojiProps) {
  return (
    <button
      {...props}
      className={cn(
        "flex size-8 items-center justify-center rounded-sm text-lg sm:size-12 sm:text-3xl data-active:bg-accent",
        className,
      )}
...
function EmojiPickerCategoryHeader({ category, ...props }: EmojiPickerListCategoryHeaderProps) {
  return (
    <div
      {...props}
      className="bg-background px-3 pt-3.5 pb-2 text-xs leading-none text-muted-foreground"
